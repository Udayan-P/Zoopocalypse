...
<style>
/* Full CSS preserved exactly as before */
body {{
...
.species-block {{
    ...
    max-width: 480px;
    text-align: center;
    border: 1px solid rgba(46, 119, 204, 0.35);
...
<body>
<div class="container">
<div class="header-card">{header_content}</div>
{content}
</div>
</body>
</html>
...
def convert_markdown_to_html(md_content):
    ...
    header_match = re.search(
        r"<h1>Feature Challenge: Identify the Animal</h1>(.*?)<hr />",
        html,
        flags=re.DOTALL,
    )
    ...
    if header_match:
        ...
        html_body = html.replace(header_match.group(0), "")
    else:
    ...
    html_body = re.sub(
        ...
        (
            ...
            '<div class="species-title">Species</div>'
            '<div class="species-value">[CENSORED]</div>'
            '</div>'
        ),
        html_body,
    )
    ...
    def convert_buttons(match):
        ...
        for href, label in links:
            out.append(f"<a class='btn' href='{href}'>{label}</a>")
        out.append("</div>")
        return "".join(out)
...
def main():
    ...
    header_html, body_html = convert_markdown_to_html(md_content)
